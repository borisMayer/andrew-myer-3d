...
export default async function handler(req: Request) {
  ...
  if (req.method === 'POST') {
    ...
    const rows = await sql`
      ...
      VALUES (${body.book_id}, ${body.currency}, ${body.amount}, ${body.is_active ?? true})
      ON CONFLICT (book_id, currency)
      DO UPDATE SET amount = ${body.amount}, is_active = ${body.is_active ?? true}
      RETURNING *
    `;
